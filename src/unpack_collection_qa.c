...
void usage ()
{
    printf ("unpack_collection_qa will read the QA band from the Landsat L4-8 "
            "input Collection product, then unpack this band into individual "
            "QA bands stored in multiple files using the user-specified base "
            "output filename.  The output bands will refer to the QA bits "
            "(from right to left), representing the QA information which is "
            "stored in the QA band.  In some cases a single bit is used to "
            "represent quality data in the QA band and in other cases two bits "
            "are used for the quality info.\n\n"
